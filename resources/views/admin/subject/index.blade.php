            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" id="simple-search" name="q" value="{{ request('q') }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                                              ...
                                              dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search subject...">
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="w-full md:w-auto flex justify-end">
                        <button type="button" data-modal-target="addSubjectModal" data-modal-toggle="addSubjectModal"
                            ...
                            @foreach ($subjects as $s)
                                <tr class="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        {{-- Tombol Action--}}
                                        <button id="subject-actions-{{ $s->id }}-button"
                                            data-dropdown-toggle="subject-actions-{{ $s->id }}" type="button" class="inline-flex items-center p-0.5 text-sm font-medium text-center
                                        ...
                                        {{-- Dropdown (Edit saja) --}}
                                        <div id="subject-actions-{{ $s->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow
                                                                dark:bg-gray-700 dark:divide-gray-600">
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                aria-labelledby="subject-actions-{{ $s->id }}-button">
                                                <li>
                                                    <button type="button" data-modal-target="editSubjectModal-{{ $s->id }}"
                                                        data-modal-toggle="editSubjectModal-{{ $s->id }}" class="w-full text-left block py-2 px-4 hover:bg-gray-100
                ...
                {{-- MODAL EDIT PER SUBJECT --}}
                @foreach ($subjects as $subject)
                    @include('admin.subject.update', ['subject' => $subject])
                @endforeach
                    ...
                    @if ($subjects->count())
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">
                                {{ $subjects->firstItem() }}-{{ $subjects->lastItem() }}
                            </span>
                            of
                            <span class="font-semibold text-gray-900 dark:text-white">
                                {{ $subjects->total() }}
